...
class TestSchemaConfigObjectCache {
  final bool? enabled;
  final int? ttl;

  const TestSchemaConfigObjectCache({
    this.enabled,
    this.ttl,
  });

  factory TestSchemaConfigObjectCache.fromJson(Map<String, dynamic> json) {
    final remaining = Map<String, dynamic>.from(json);
    final enabled = json['enabled'] as bool?;
    remaining.remove('enabled');
    final ttl = json['ttl'] as int?;
    remaining.remove('ttl');
    return TestSchemaConfigObjectCache(
      enabled: enabled,
      ttl: ttl,
    );
  }
