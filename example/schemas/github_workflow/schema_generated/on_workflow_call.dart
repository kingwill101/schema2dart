...
import 'on_workflow_call_inputs.dart';
import 'on_workflow_call_outputs.dart';
import 'validation_error.dart';

/// Allows workflows to be reused by other workflows.
class OnWorkflowCall {
  ...
  final OnWorkflowCallInputs? inputs;
  /// When using the workflow_call keyword, you can optionally specify inputs that are passed to the called workflow from the caller workflow.
  final OnWorkflowCallOutputs? outputs;
  ...
  const OnWorkflowCall({
    this.inputs,
    this.outputs,
    this.secrets,
  });

  factory OnWorkflowCall.fromJson(Map<String, dynamic> json) {
    final remaining = Map<String, dynamic>.from(json);
    final inputs = json['inputs'] == null ? null : OnWorkflowCallInputs.fromJson((json['inputs'] as Map).cast<String, dynamic>());
    remaining.remove('inputs');
    final outputs = json['outputs'] == null ? null : OnWorkflowCallOutputs.fromJson((json['outputs'] as Map).cast<String, dynamic>());
    remaining.remove('outputs');
    final secrets = json['secrets'];
    remaining.remove('secrets');
    return OnWorkflowCall(
      inputs: inputs,
      outputs: outputs,
      secrets: secrets,
    );
