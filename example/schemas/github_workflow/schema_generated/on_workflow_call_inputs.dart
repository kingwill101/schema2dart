...
import 'on_workflow_call_inputs_pattern_property1.dart';
...
class OnWorkflowCallInputs {
  final Map<String, OnWorkflowCallInputsPatternProperty1>? patternProperties;

  const OnWorkflowCallInputs({
    this.patternProperties,
  });

  factory OnWorkflowCallInputs.fromJson(Map<String, dynamic> json) {
    final remaining = Map<String, dynamic>.from(json);
    var unmatched = Map<String, dynamic>.from(remaining);
    Map<String, OnWorkflowCallInputsPatternProperty1>? patternPropertiesValue;
    {
      final patternPropertiesMap = <String, OnWorkflowCallInputsPatternProperty1>{};
      ...
      for (final entry in unmatched.entries) {
        ...
        if (!matched && RegExp('^[_a-zA-Z][a-zA-Z0-9_-]*\$').hasMatch(key)) {
          patternPropertiesMap[key] = OnWorkflowCallInputsPatternProperty1.fromJson((value as Map).cast<String, dynamic>());
          matched = true;
        }
        if (!matched) {
