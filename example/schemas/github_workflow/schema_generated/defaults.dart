// GENERATED CODE - DO NOT MODIFY BY HAND
// Source: example/schemas/github_workflow/schema.json
// Generated by json_schema2dart

import 'defaults_run.dart';
import 'validation_error.dart';

class Defaults {
  /// Constraints: minProperties: 1
  final DefaultsRun? run;

  const Defaults({this.run});

  factory Defaults.fromJson(Map<String, dynamic> json) {
    final remaining = Map<String, dynamic>.from(json);
    final run = json['run'] == null
        ? null
        : DefaultsRun.fromJson((json['run'] as Map).cast<String, dynamic>());
    remaining.remove('run');
    var unmatched = Map<String, dynamic>.from(remaining);
    if (unmatched.isNotEmpty) {
      final unexpected = unmatched.keys.join(', ');
      throw ArgumentError('Unexpected additional properties: $unexpected');
    }
    return Defaults(run: run);
  }

  Map<String, dynamic> toJson() {
    final map = <String, dynamic>{};
    if (run != null) map['run'] = run!.toJson();
    return map;
  }

  void validate({String pointer = '', ValidationContext? context}) {
    final _ptr0 = appendJsonPointer(pointer, 'run');
    final _value0 = run;
    if (_value0 != null) {
      context?.markProperty(pointer, 'run');
      final _propertyCountp0 = _value0.toJson().length;
      if (_propertyCountp0 < 1) {
        throwValidationError(
          _ptr0,
          'minProperties',
          'Expected at least 1 properties but found ' +
              _propertyCountp0.toString() +
              '.',
        );
      }
    }
  }
}
