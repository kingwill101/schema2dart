...
sealed class NormalJobConcurrency {
  ...
  factory NormalJobConcurrency.fromJson(dynamic json) {
    ...
    if (json is! Map<String, dynamic>) {
      throw ArgumentError(
        'Invalid NormalJobConcurrency value: ${json.runtimeType}',
      );
    }
    final keys = json.keys.toSet();
    final sortedKeys = keys.toList()..sort();
    final requiredMatches =
        <NormalJobConcurrency Function(Map<String, dynamic>)>[];
    final requiredMatchNames = <String>[];
    if (keys.contains('group')) {
      requiredMatches.add(NormalJobConcurrencyConcurrency.fromJson);
      requiredMatchNames.add('NormalJobConcurrencyConcurrency');
    }
    ...
    if (requiredMatches.length > 1) {
      throw ArgumentError(
        'Ambiguous NormalJobConcurrency variant matched required-property heuristics: ${requiredMatchNames.join(', ')}',
      );
    }
    return NormalJobConcurrencyConcurrency.fromJson(json);
  }
...
class NormalJobConcurrencyConcurrency extends NormalJobConcurrency {
  ...
  const NormalJobConcurrencyConcurrency({
    this.cancelInProgress,
    required this.group,
  }) : super();

  factory NormalJobConcurrencyConcurrency.fromJson(Map<String, dynamic> json) {
    ...
    return NormalJobConcurrencyConcurrency(
