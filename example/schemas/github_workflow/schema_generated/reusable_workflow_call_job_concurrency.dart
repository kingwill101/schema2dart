...
sealed class ReusableWorkflowCallJobConcurrency {
  ...
  factory ReusableWorkflowCallJobConcurrency.fromJson(dynamic json) {
    ...
    if (json is! Map<String, dynamic>) {
      throw ArgumentError(
        'Invalid ReusableWorkflowCallJobConcurrency value: ${json.runtimeType}',
      );
    }
    final keys = json.keys.toSet();
    final sortedKeys = keys.toList()..sort();
    final requiredMatches =
        <ReusableWorkflowCallJobConcurrency Function(Map<String, dynamic>)>[];
    final requiredMatchNames = <String>[];
    if (keys.contains('group')) {
      requiredMatches.add(Concurrency.fromJson);
      requiredMatchNames.add('Concurrency');
    }
    ...
    if (requiredMatches.length > 1) {
      throw ArgumentError(
        'Ambiguous ReusableWorkflowCallJobConcurrency variant matched required-property heuristics: ${requiredMatchNames.join(', ')}',
      );
    }
    return Concurrency.fromJson(json);
  }

  dynamic toJson();
}

class ReusableWorkflowCallJobConcurrencyString
    extends ReusableWorkflowCallJobConcurrency {
  final String value;
...
class Concurrency extends ReusableWorkflowCallJobConcurrency {
  ...
  const Concurrency({this.cancelInProgress, required this.group}) : super();
  ...
  factory Concurrency.fromJson(Map<String, dynamic> json) {
    ...
    return Concurrency(cancelInProgress: cancelInProgress, group: group);
  }
